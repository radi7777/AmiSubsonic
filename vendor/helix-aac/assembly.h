...
/*  toolchain:           m68k-amigaos-gcc (AmiSubsonic, 2026)
    target architecture: 68020 and up, big endian, no FPU

    AmiSubsonic modification, the only change to this file: the generic
    C block below lays out U64 little endian (lo32 first). On the 68k the
    high word comes first, so the SBR energy and QMF code would read the
    halves swapped. gcc emits a single muls.l for the 32x32->64 multiply
    on 68020, so no inline assembly is needed.
*/
